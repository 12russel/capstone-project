...
<div class="container shadow">
    <div class="row">
        ...
        <div class="col-md-4 my-3">
            <a href="{{ route('instructor.course.category-create') }}" class="btn btn-primary float-md-end">Create Category</a>
        </div>
    </div>

    <!-- Live Search Bar -->
    <form action="{{ route('instructor.course.category') }}" method="GET" class="my-3 row">
    ...
    <table class="table" id="categoryTable">
        <thead>
            <tr>
                <th onclick="sortTable(0)">Category <span id="categorySortIcon">&#8593;</span></th>
                <th onclick="sortTable(1)" class="text-center">Course <span id="courseSortIcon">&#8593;</span></th>
        ...
        <tbody>
            @foreach ($categories as $course)
                <tr>
                    ...
                    <td class="text-center">{{ $course->paid == 1 ? 'Paid' : '' }}</td>
                    <td class="text-center">{{ $course->sales->sum('amount') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
<script>
    ...
    function updateSortIcons(columnIndex, direction) {
        var sortIconIds = ['categorySortIcon', 'courseSortIcon'];
        // Reset all icons
